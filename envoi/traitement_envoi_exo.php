...
if (!isset($_POST['Quoi1']) OR trim($_POST['Quoi1'])=='')
	{
	  $prenom1 = '';
	}
else
	{ // Nom non vide -> à traiter
		$quoi1 = $_POST['Quoi1'];
		$quoi1 = traitement($quoi1);
	}

if (!isset($_POST['Message1']) OR trim($_POST['Message1'])=='')
	{
	  $message1 = '';
	}
else
...
if ($formulaire_valide)
{
		...
		$fichier= $horodatage.'_'.$nom1.'-'.$prenom1.'_'.$quoi1.'_'.$_SERVER["REMOTE_ADDR"].'.'.$extension;
			
		//Déplacement du fichier de la zone tmp vers la zone définitive
		if (move_uploaded_file($_FILES['fichier']['tmp_name'], $dossier.'/'.$pourQui.'/'.$fichier ))
		{
